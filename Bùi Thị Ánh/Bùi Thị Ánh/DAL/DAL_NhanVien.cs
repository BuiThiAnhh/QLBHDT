...
namespace DAL
{
    public class DAL_NhanVien : DBConnect
    {
        ...
        {
            string sql = string.Format("Insert into NhanVien values('{0}', N'{1}', N'{2}', '{3}')", nv.MaNV, nv.TenNV, nv.DiaChi, nv.SoDienThoai);
            thucthisql(sql);
            return true;
        }
        ...
        {
            string sql = string.Format("Update NhanVien Set TenNV = N'{0}', DiaChi = N'{1}', SoDienThoai = '{2}' Where MaNV = '{3}'", nv.TenNV, nv.DiaChi, nv.SoDienThoai, nv.MaNV);
            thucthisql(sql);
            return true;
        }
        public string valuecbbNV(int ma)
        {
